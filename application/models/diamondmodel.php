...
*/					
			...
			$cutarray = array('0' => 'Good','1' => 'Very Good','2' => 'Ideal','3' => 'Excellent');
...
//		. $qwhere;
	...
	function getSearch( $page =1 , $rp = 10 ,$sortname = 'title' ,$sortorder = 'desc' ,$query= '' , $qtype = 'title' , $oid = '', $searchtype = ''){
		...
		//file_put_contents("somefile.sql.txt",$searchtype . "\n",FILE_APPEND);	
		...
		$results = array();
		  ...
		  else
		  {
			...
			if(($this->session->userdata('searchminprice'))!=null) $qwhere .= " AND price >= ". $this->session->userdata('searchminprice');
			if(($this->session->userdata('searchmaxprice'))!=null) $qwhere .= " AND price <= ". $this->session->userdata('searchmaxprice');
			if(($this->session->userdata('depthmin'))!=null) $qwhere .= " AND Depth >= ". $this->session->userdata('depthmin');
			if(($this->session->userdata('depthmax'))!=null) $qwhere .= " AND Depth <= ". $this->session->userdata('depthmax');
			if(($this->session->userdata('caratmin'))!=null) $qwhere .= " AND carat >= ". $this->session->userdata('caratmin');
			if(($this->session->userdata('caratmax'))!=null) $qwhere .= " AND carat <= ". $this->session->userdata('caratmax');

            $shape = '';
            if(($this->session->userdata('shape'))!=null) $shape =  $this->session->userdata('shape');
//            file_put_contents("somefile.sql.txt","shape:".$shape . "\n",FILE_APPEND);
            $shapestr = '';
...
//            file_put_contents("somefile.sql.txt","shapestr:".$shapestr . "\n",FILE_APPEND);    
//                if($searchtype == 'tothreestone' || $searchtype == 'addpendantsetings3stone') $qwhere .=  "       AND        shape in ('B','PR','E')";
                $qwhere .=  " AND shape in ( " . substr($shapestr  , 0 , (strlen($shapestr) -1)) . ")";
            }
            else {
                $qwhere .=  " AND shape not in ('PR','AS','E','R','O','M','P','H','B','C')";
            }
			...
			if($cutmin <= $cutmax && $cutmax < count($cutarray)) {
				for($i= $cutmin; $i<=$cutmax; $i++)
				if($cutarray[$i] != '')$cutstr .= "'".$cutarray[$i]."',";
				$cutstr = substr($cutstr , 0 , (strlen($cutstr) -1));
			}
		...
		//	if(($this->session->userdata('ispremium'))!=null){ $cutstr = "'Premium'"; }
			...
			$claritys = array('FL','IF','VVS1','VVS2','VS1','VS2','SI1','SI2','SI3');
			$clarity = '';  
			$claritymin = 0;
...
// PAPILLON UPDATE 20120411
		...
		$resultminmax = $this->db->query("SELECT ROUND(min(price)) dbminprice, ROUND(max(price)) dbmaxprice, min(carat) dbmincarat, max(carat) dbmaxcarat FROM " . $this->config->item('table_perfix') . "products");
...
// END PAPILLON UPDATE 20120411
	...
	function getMaxPrice(){
		$qry = "SELECT max( price ) AS maxx FROM ".config_item('table_perfix')."products";
		$result = 	$this->db->query($qry);
		$price = $result->result_array();	
		return $price[0]['max'];
	}
	...
	function getMinPrice(){
		$qry = "SELECT min( price ) AS min FROM ".config_item('table_perfix')."products";
		$result = 	$this->db->query($qry);
		$price = $result->result_array();	
		return $price[0]['min'];
	}
	...
	function getDetailsByLot($lot){
		$qry = "SELECT * FROM ".config_item('table_perfix')."products WHERE lot = '".$lot."'";
//echo($qry);
		$result = 	$this->db->query($qry);
		$return = $result->result_array();	
	...
	function getDetailsByLotproduct($lot){
		$qry = "SELECT * FROM products WHERE ProductID = '".$lot."'";
		$result = 	$this->db->query($qry);
		$return = $result->result_array();	
		return isset($return[0]) ? $return[0] : false;
	}
	...
	function getMinPricePerCarat(){
		$qry = "SELECT min( pricepercrt ) AS min FROM ".config_item('table_perfix')."products";
		$result = 	$this->db->query($qry);
		$price = $result->result_array();	
		return $price[0]['min'];
	}
	...
	function getCountDiamond($minPrice,$maxPrice,$shape=''){
		$qry = "SELECT count(lot) as cnt FROM ".config_item('table_perfix')."products";
		if($minPrice!='' && $maxPrice!=''){
			$qry .= " WHERE price >=".$minPrice." and price <=".$maxPrice;
		}				
		$result = 	$this->db->query($qry);
	...
	function getMinMaxCarat(){		
		$qry = "SELECT min(carat) as min, max(carat) as max FROM ".config_item('table_perfix')."products";
		$return = 	$this->db->query($qry);
		$result = $return->result_array();	
		return $result[0];
	}
...
//exit();
	...
	function getSidestoneByCenterLot($diamond = ''){
		...
		$qry = "SELECT * FROM ".config_item('table_perfix')."products WHERE ".$condition;
//print_r($diamond);
//die($qry);
		$result = 	$this->db->query($qry);	
		$return = $result->result_array(); 
	...
	function getAllSideStones($diamond = '',$start = 0){
		$results = array();
		$condition =  $this->sidestonecondition($diamond);		
		$qry = "SELECT * FROM ".config_item('table_perfix')."products WHERE ".$condition . " limit ".$start." ,10";
		$result = 	$this->db->query($qry);
		$results['result'] = $result->result_array();
		$qry2 = "SELECT *	FROM ".config_item('table_perfix')."products	WHERE ".$condition;
	...
	function get2sidestone($tablemin,$tablemax,$depthmin,$depthmax,$sidelot){
		...
		//$condition =  $this->sidestonecondition($diamond);
		$qry = "SELECT * FROM ".config_item('table_perfix')."products WHERE carat < ".$diamond['carat']." and TablePercent >=".$tablemin." and TablePercent <=".$tablemax."  and Depth >=".$depthmin." and Depth <=".$depthmax." order by carat DESC";				
		$result = 	$this->db->query($qry);
		$return = $result->result_array();
		return isset($return) ? $return : false;
	}	
	...
	function getPairSidestone($carat, $color, $clarity, $tablecondition, $depthcondition){
		$qry = "SELECT * 	FROM ".config_item('table_perfix')."products WHERE carat = ".$carat." and color = '".$color."' and clarity = '".$clarity."' and ".$tablecondition." and ".$depthcondition;
...
//	$this->db->update('error', array('msg' => $qry . '------------'));		
	...
	function getEarRingSideStone($diamond = ''){
		...
		$minpricefilter = floatval($this->commonmodel->db_config('minpricefilter'));
		$qry = "SELECT * 	FROM ".config_item('table_perfix')."products WHERE ".$condition. " and price > $minpricefilter ORDER BY price ASC";
//print_r($diamond);
//echo($qry);	
//die($qry);
		$result = 	$this->db->query($qry);
