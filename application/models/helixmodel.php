...
class Helixmodel extends  Model {
	...
	function getStullerprices( $page =1 , $rp = 10 ,$sortname = 'pricefrom' ,$sortorder = 'asc' ,$query= '' , $qtype = 'pricefrom' , $oid = ''){
		$results = array();
		...
		$sql = 'SELECT  * FROM  '. $this->config->item('table_perfix').'stuller_prices where 1=1 '. $qwhere . ' ' . $sort . ' '. $limit;
		//var_dump($sql); 
	 	$result = $this->db->query($sql);
		$results['result']  = $result->result_array();	
		$sql2 = 'SELECT  rowid FROM  '. $this->config->item('table_perfix').'stuller_prices  where 1=1 '. $qwhere;
		$result2 = $this->db->query($sql2);
    	$results['count']  = $result2->num_rows();
	...
	function getQualityprices( $page =1 , $rp = 10 ,$sortname = 'pricefrom' ,$sortorder = 'asc' ,$query= '' , $qtype = 'pricefrom' , $oid = ''){
		$results = array();
		...
		$sql = 'SELECT  * FROM  '. $this->config->item('table_perfix').'quality_prices where 1=1 '. $qwhere . ' ' . $sort . ' '. $limit;
		//var_dump($sql); 
	 	$result = $this->db->query($sql);
		$results['result']  = $result->result_array();	
		$sql2 = 'SELECT  rowid FROM  '. $this->config->item('table_perfix').'quality_prices  where 1=1 '. $qwhere;
		$result2 = $this->db->query($sql2);
    	$results['count']  = $result2->num_rows();
	...
	function stuller_helixprices($post , $action = 'view' , $id =0)
  	{
		  	...
		  	if(is_array($post)){		
				 ...
				 if( ($pricefrom < $priceto ) && ($pricefrom >0) && ($priceto > 0)){
			 		...
			 		if($action == 'add')
			 		{
				 		...
				 		$isinsert = $this->db->insert($this->config->item('table_perfix').'stuller_prices',
				 		array(
		  						  'pricefrom' => $pricefrom,
		  						  'priceto'	  => $priceto,
		  						  'rate'	  => $rate
			 		...
			 		$isinsert = $this->db->update($this->config->item('table_perfix').'stuller_prices',
			 		array(			  
			  					  'pricefrom' => $pricefrom,
		  						  'priceto'	  => $priceto,
		  						  'rate'	  => $rate
				 	...
				 	if($isinsert) $retuen['success'] .= '<h1 class="success">Price Range has been '.ucfirst($action).'ed .</h1><br><br><center><a href="'.config_item('base_url').'admin/helixpriceStuller">Continue</a></center><br /> ';
				 }
				 else{
 ...
 function quality_helixprices($post , $action = 'view' , $id =0)
  	{
		  	...
		  	if(is_array($post)){		
				 ...
				 if( ($pricefrom < $priceto ) && ($pricefrom >0) && ($priceto > 0)){
			 		...
			 		if($action == 'add')
			 		{
				 		...
				 		$isinsert = $this->db->insert($this->config->item('table_perfix').'quality_prices',
				 		array(
		  						  'pricefrom' => $pricefrom,
		  						  'priceto'	  => $priceto,
		  						  'rate'	  => $rate
			 		...
			 		{
			 		$this->db->where('rowid' , $id);
			 		$isinsert = $this->db->update($this->config->item('table_perfix').'quality_prices',
			 		array(			  
			  					  'pricefrom' => $pricefrom,
		  						  'priceto'	  => $priceto,
		  						  'rate'	  => $rate
				 	...
				 	if($isinsert) $retuen['success'] .= '<h1 class="success">Price Range has been '.ucfirst($action).'ed .</h1><br><br><center><a href="'.config_item('base_url').'admin/helixpriceQuality">Continue</a></center><br /> ';
				 }
				 else{
				 	$retuen['error'] .= '<h1 class="error">Invalid price range ! Try Again</h1><br /> ';
				 }
	...
	function getPriceByStullerID($id = 0){
 	$qry = "SELECT *
				FROM ".config_item('table_perfix')."stuller_prices
				WHERE rowid = '".$id."'
				";
		$result = 	$this->db->query($qry);
 ...
 function getPriceByQualityID($id = 0){
 	$qry = "SELECT *
				FROM ".config_item('table_perfix')."quality_prices
				WHERE rowid = '".$id."'
				";
		$result = 	$this->db->query($qry);
		$return = $result->result_array();	
