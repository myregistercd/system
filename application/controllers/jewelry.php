<?php class Jewelry extends Controller {
	...
	function getMaxPrice(){
		$qry = "SELECT max( price ) AS max FROM ".config_item('table_perfix')."products";
	...
	function getpendantsettingsresult($isPlatinum = '', $isYellowgold = '', $isWhitegold = '', $isSolitaire = '', $isThreestone = ''){
		...
		if(isset($results)){
			...
			foreach ($results as $result){
   			    ...
   			    $img = file_exists($pth.'/'.$result['small_image']) ? config_item('base_url').$result['small_image'] : config_item('base_url').'images/rings/noimage.jpg';

				if(($count%3) == 0)$returnhtml .='</div><div class="clear"></div><div style="border-bottom:5px #0000000 solid;"> ';

				$returnhtml .='

									<div style=" color:#fffff; float:left;padding-bottom:33px;padding-top: 15px; " class="floatl ringbox2 txtcenter2"> 				      		
